...
Has the real install manifest + service worker (so browsers offer "Add to
Home Screen" / "Install app")
Displays a branded splash screen, then embeds your live Streamlit app
...
Purpose
index.html
The shell page — splash screen + iframe embed
manifest.json
App name, icon, colors — what Android/Chrome use to install
service-worker.js
Minimal worker, required for install eligibility
icons/icon-192.png, icons/icon-512.png
Placeholder icons — replace these
...
Export your real WealthLens logo as two square PNGs: 192x192 and
512x512 pixels
Replace icons/icon-192.png and icons/icon-512.png with them (same
filenames)
Keep some padding around the logo — Android's "maskable icon" spec crops
into a circle/rounded-square, so anything near the edges can get clipped
Deploy to GitHub Pages (free, ~5 minutes)
In your web-whelthlens GitHub repo (or a new small repo — either
...
Repo → Settings → Pages
Source: Deploy from a branch → Branch: main → Folder: /pwa (or
...
Wait ~1 minute for it to go live, then open that URL on your phone
Installing it
